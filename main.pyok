...
def create_voice_inline_keyboard():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("Codka Qulaa Sada", callback_data="voice_qulaa_sada")
    )
    return keyboard
...
def send_and_pin_welcome(chat_id):
    try:
        keyboard = create_voice_inline_keyboard()
        msg = bot.send_message(
            chat_id,
            "👋 Soo dhawow!\n\nWaxaan ahay Somali Text to Speech Bot 🎙️\nDooro codka aad rabto (Cabdi, Muno, ama Codka Qulaa Sada), kadibna ii soo dir qoraalka.\n\n👇 Dooro codka:",
            reply_markup=keyboard
        )
        try:
            bot.pin_chat_message(chat_id, msg.message_id)
            pinned_welcome_ids[str(chat_id)] = msg.message_id
        except Exception:
            pass
    except Exception:
        pass
...
def start(message):
    send_and_pin_welcome(message.chat.id)
...
def callback_set_voice(call):
    ...
    if call.data == "voice_male":
        CURRENT_VOICE_NAME = "so-SO-MuuseNeural"
        CURRENT_VOICE_LABEL = "Cabdi"
        user_rate_settings[uid] = user_rate_settings.get(uid, 0)
        user_pitch_settings[uid] = user_pitch_settings.get(uid, 0)
        bot.send_message(call.message.chat.id, "Hadda codka waa Cabdi")
    elif call.data == "voice_female":
        CURRENT_VOICE_NAME = "so-SO-UbaxNeural"
        CURRENT_VOICE_LABEL = "Muno"
        user_rate_settings[uid] = user_rate_settings.get(uid, 0)
        user_pitch_settings[uid] = user_pitch_settings.get(uid, 0)
        bot.send_message(call.message.chat.id, "Hadda codka waa Muno")
    elif call.data == "voice_qulaa_sada":
        CURRENT_VOICE_NAME = "so-SO-MuuseNeural"
        CURRENT_VOICE_LABEL = "Muuse (Qulaa Sada)"
        user_rate_settings[uid] = 35
        user_pitch_settings[uid] = 65
        bot.send_message(call.message.chat.id, "Hadda codka waa Muuse (Codka Qulaa Sada). Pitch: 65, Rate: 35")
    try:
...
def handle_text(message):
    ...
    if chat_id_str not in pinned_welcome_ids:
        send_and_pin_welcome(message.chat.id)
