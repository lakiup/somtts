...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8380607635:AAHTqi2MS0K4PwtWZlSjZrvQfg1Ja8GMeis")
...
def create_voice_keyboard():
    ...
    keyboard.add(KeyboardButton("⚙️ Xawaare"), KeyboardButton("🎵 Pitch"))
    return keyboard

def rate_keyboard(user_id, current):
    kb = InlineKeyboardMarkup(row_width=3)
    kb.add(
        InlineKeyboardButton("➖", callback_data=f"rate_down_{user_id}"),
        InlineKeyboardButton(f"Rate: {current}", callback_data="noop"),
        InlineKeyboardButton("➕", callback_data=f"rate_up_{user_id}")
    )
    return kb

def pitch_keyboard(user_id, current):
    kb = InlineKeyboardMarkup(row_width=3)
    kb.add(
        InlineKeyboardButton("➖", callback_data=f"pitch_down_{user_id}"),
        InlineKeyboardButton(f"Pitch: {current}", callback_data="noop"),
        InlineKeyboardButton("➕", callback_data=f"pitch_up_{user_id}")
    )
    return kb
...
@bot.message_handler(func=lambda m: m.text == "⚙️ Xawaare")
def cmd_rate_button(message):
    user_id = str(message.from_user.id)
    current = user_rate_settings.get(user_id, 0)
    bot.send_message(
        message.chat.id,
        "Hagaaji xawaaraha codka:",
        reply_markup=rate_keyboard(user_id, current),
        reply_to_message_id=message.message_id
    )

@bot.message_handler(func=lambda m: m.text == "🎵 Pitch")
def cmd_pitch_button(message):
    user_id = str(message.from_user.id)
    current = user_pitch_settings.get(user_id, 0)
    bot.send_message(
        message.chat.id,
        "Hagaaji pitch-ka codka:",
        reply_markup=pitch_keyboard(user_id, current),
        reply_to_message_id=message.message_id
    )

@bot.callback_query_handler(func=lambda call: call.data and ("rate_" in call.data or "pitch_" in call.data or call.data == "noop"))
def slider_handler(call):
    ...
    action_type = parts[0]
    action = parts[1]
    uid = parts[2] if len(parts) > 2 else str(call.from_user.id)
    
    if action_type == "rate":
        val = user_rate_settings.get(uid, 0)
        val += 5 if action == "up" else -5
        ...
        try:
            bot.edit_message_reply_markup(
                ...
                reply_markup=rate_keyboard(uid, val)
            )
        except:
            pass
        ...
        try:
            bot.edit_message_reply_markup(
                call.message.chat.id,
                call.message.message_id,
                reply_markup=pitch_keyboard(uid, val)
            )
        except:
            pass
