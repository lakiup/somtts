...
app = Client(
    ...
    api_hash=API_HASH, 
    bot_token=BOT_TOKEN,
    workers=20 # Waxay kordhisaa awoodda bot-ka
)
...
async def progress(current, total, message, start_time):
    now = time.time()
    diff = now - start_time
    if diff < 2: # Waxaan kordhinay waqtiga si uusan Telegram 'Flood' noo dhihin
        return
...
async def handler(client, message):
    ...
    try:
        ...
        # Halkan waxaan ku kordhinay speed-ka upload-ka
        ...
        print(f"Error: {e}")
        await msg.edit("Error ayaa dhacay. Hubi linkiga ama tijaabi mar kale.")
